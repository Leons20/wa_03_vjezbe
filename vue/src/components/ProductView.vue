...
import { dodajUKosaricu } from '../kosarica.js';
import { dohvatiKosaricu } from '../kosarica.js';
...
const brojProizvoda = computed(() => dohvatiKosaricu().length);
...
const posaljiNarudzbu = () => {
    dodajUKosaricu(proizvod.value);
    alert('Proizvod dodan u košaricu!');
    router.push('/proizvodi'); // Vraća korisnika na početnu stranicu
};

const naruciProizvode = async () => {
    const narudzba = dohvatiKosaricu();
    await axios.post('http://localhost:3000/narudzbe', narudzba);
...
<template>
    ...
    <div class="pt-6">
      <nav aria-label="Breadcrumb">
        ...
        <div>
          <h1>Proizvodi</h1>
          <p>Proizvoda u košarici: {{ brojProizvoda }}</p>
          <button @click="naruciProizvode" class="bg-green-500">Naruči proizvode</button>
        </div>
      </nav>
        ...
        <div class="mt-4 lg:row-span-3 lg:mt-0">
          ...
          <form class="mt-10">
            ...
            <div>
              <h3>Boje:</h3>
                <ul>
                    <li v-for="boja in proizvod.dostupne_boje" :key="boja">{{ boja }}</li>
                </ul>
                <h3>Karakteristike:</h3>
                <ul>
                    <li v-for="karakteristika in proizvod.karakteristike" :key="karakteristika">{{ karakteristika }}</li>
                </ul>
